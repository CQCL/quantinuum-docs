r"""BQPE calculation using a noise aware prior."""
import numpy as np
import phayes
...
def noisy_backend(cx_err, ro_err, n_qubits):
    ...
    # Add CX error.
    error_1 = noise.depolarizing_error(cx_err, 2)
    for i in range(n_qubits):
        for j in range(n_qubits):
            if i != j:
                noise_model.add_quantum_error(error_1, ["cx"], [i, j])
    # Add RO error.
    probabilities = [[1 - ro_err, ro_err], [ro_err, 1 - ro_err]]
    readout_error = ReadoutError(probabilities)
    for i in range(n_qubits):
        noise_model.add_readout_error(readout_error, qubits=[i])
    # Construct noisy backend.
...
protocol = IterativePhaseEstimationQuantinuum(
    ...
    encoding_method=CircuitEncoderQuantinuum.PLAIN,

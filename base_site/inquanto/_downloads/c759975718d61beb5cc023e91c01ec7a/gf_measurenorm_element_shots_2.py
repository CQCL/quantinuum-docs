r"""Computation (shots for Lanczos, shots for ground state) one element of GF of Hubbard dimer."""
...
vqe = (
    AlgorithmVQE(
        ...
        minimizer=MinimizerScipy(method=OptimizationMethod.L_BFGS_B_smooth),
        initial_parameters=parameters,
    )
    .build(
        protocol_objective=PauliAveraging(
            AerBackend(),
...
# noisy ansatz due to finite samples in vqe
...
print(
    "Number of measurement circuits for lanczos: "
    + str(len(pa_protocol.get_circuits()))
)
...
pyplot.savefig("example_gf_measurenorm_element_shots_2.png")

r"""A AlgorithmMcLachlanImagTime time evolution simulation"""
...
# For this example, we'll propagate a Hubbard dimer in imaginary time and examine the relaxation of the system as the electrons move from the initial state.
...
# Hamiltonian and number operators return by the above defined method
...
# Instantiate the variation time evo class using the integrator, hamiltonian and state
algodeint = AlgorithmMcLachlanImagTime(
    integrator,
    hamiltonian,
    ansatz,
...
# Specify the protocol that builds the needed metric Tensor and Derivative computables
...
# these figures show the relaxation of the system and the movement of electrons from the 1001 state
plotting(
    time,
    evs,
    solution,
